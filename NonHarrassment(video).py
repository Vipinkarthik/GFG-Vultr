...
non_harassment_model = load_model('non_harassment_model.h5')
...
def predict_non_harassment(video_path):
    cap = cv2.VideoCapture(video_path)
    non_harassment_detected = False
    ...
    while cap.isOpened():
        ...
        prediction = non_harassment_model.predict(processed_frame)
        
        if prediction[0][1] > 0.5:
            non_harassment_detected = True
            break

    cap.release()
    return non_harassment_detected
...
def upload_video():
    ...
    non_harassment_detected = predict_non_harassment(video_path)

    if non_harassment_detected:
...
root = tk.Tk()
root.title("Non-Harassment Detection")

header_frame = tk.Frame(root, pady=10)
header_frame.pack(fill=tk.X)

header_label = tk.Label(header_frame, text="Non-Harassment Detection System", font=("Helvetica", 16, "bold"))
header_label.pack()
